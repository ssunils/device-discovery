...
export class SignalTracker {
  ...
  private connectWebSocket() {
    ...
    const wsUrl =
      ...
      "/v1/receive/" +
      encodeURIComponent(this.senderNumber);
  ...
  private processJsonRpcMessage(message: any) {
    ...
    if (envelope.receiptMessage?.isDelivery) {
      ...
      if (
        ...
        sourceNumber === this.targetNumber
      ) {
  ...
  private async sendReactionProbe() {
    ...
    try {
      const response = await fetch(
        `${this.apiUrl}/v1/reactions/${encodeURIComponent(this.senderNumber)}`,
        {
          method: "POST",
      ...
      if (response.ok || response.status === 204) {
        ...
        logger.debug(
          `Failed to send reaction probe: ${response.status} - ${errorText}`
        );
      }
    } catch (err) {
  ...
  private async sendMessageProbe() {
    try {
      ...
      if (response.ok) {
        ...
        const errorText = await response.text();
        logger.debug(
          `Failed to send message probe: ${response.status} - ${errorText}`
        );
      }
    } catch (err) {
  ...
  private markDeviceOffline(identifier: string, timeout: number) {
    ...
    logger.info(
      `Device ${identifier} marked as OFFLINE (no receipt after ${timeout}ms)`
    );
    this.sendUpdate();
  }
  ...
  private determineDeviceState(identifier: string) {
    ...
    if (metrics.state === "OFFLINE") {
      if (metrics.lastRtt <= 5000 && metrics.recentRtts.length > 0) {
        logger.debug(
          `Device ${identifier} came back online (RTT: ${metrics.lastRtt}ms)`
        );
      } else {
        return;
      }
    ...
    const stateColor =
      metrics.state === "Online"
        ...
        : "⚪";
    logger.info(
      `${stateColor} ${identifier}: ${metrics.state} (RTT: ${
        metrics.lastRtt
      }ms, Avg: ${movingAvg.toFixed(0)}ms)`
    );
  }
      ...
      ([id, metrics]) => ({
        ...
        avg:
          metrics.recentRtts.length > 0
            ? metrics.recentRtts.reduce((a, b) => a + b, 0) /
 ...
 */
export function getSignalQrLinkUrl(
  apiUrl: string,
  deviceName: string = "activity-tracker"
): string {
  return `${apiUrl}/v1/qrcodelink?device_name=${encodeURIComponent(
    deviceName
  )}`;
}
...
export async function checkSignalNumber(
  ...
  try {
    const response = await fetch(
      `${apiUrl}/v1/search/${encodeURIComponent(
        senderNumber
      )}?numbers=${encodeURIComponent(targetNumber)}`,
      ...
      if (Array.isArray(results) && results.length > 0) {
        ...
        if (result.registered) {
          ...
          return {
            registered: false,
            error:
              "Number is not registered on Signal or has privacy settings blocking discovery",
          };
        }
      }
    }
    ...
    return {
      registered: false,
      error: "Failed to check Signal registration status",
    };
  } catch (err) {
