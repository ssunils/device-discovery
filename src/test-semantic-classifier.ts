...
for (const [phone, classification] of results) {
  ...
  console.log(`  Predicted:   ${classification.osType} ${passed ? '✅' : '❌'}`);
  console.log(`  Confidence:  ${(classification.confidence * 100).toFixed(1)}%`);
  console.log(`  Signals:     reg=${classification.signals.registrationId}, pre=${classification.signals.preKeyId}, signed=${classification.signals.signedKeyId}, chains=${classification.signals.chainCount}, prefix=${classification.signals.lidPrefix}`);
  console.log('---');
}

console.log(`\nFinal Score: ${correct}/${total} (${((correct/total)*100).toFixed(1)}%)`);
